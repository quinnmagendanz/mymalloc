...
int main(int argc, char** argv) {
  ...
  double total_log_throughput, total_log_util, average_log_util, average_log_throughput,
         log_p1, log_p2, perfindex;
  int numcorrect;
  ...
  while ((c = getopt(argc, argv, "f:t:hvVgcb")) != EOF) {
    ...
    case 't': /* Directory where the traces are located */
      ...
      if (tracedir[strlen(tracedir) - 1] != '/') {
        strncat(tracedir, "/", MAXLINE - strlen(tracedir) - 1);  /* path always ends with "/" */
      }
      break;
  ...
  for (i = 0; i < num_tracefiles; i++) {
    if (mm_stats[i].valid) {
      ...
      total_log_util += log(mm_stats[i].util); /* util is greater than 0 so no overflow issues */
  ...
  printf("# GeometricMean(%f (util),  %f (tput))  =  %f\n",
         100.0 * exp(average_log_util), 100.0 * exp(average_log_throughput), perfindex);
